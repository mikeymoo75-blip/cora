...
async function yahooQuotes(): Promise<Quote[]> {
...
async function pumpQuotes(solUsd: number): Promise<Quote[]> {
  ...
  return coins.slice(0, 12).map((c, i) => {
    ...
    const symbol = (c.symbol || "MEME").toUpperCase().slice(0, 8);
    const id = `pump:${c.mint || symbol}`;
    return {
      id,
      symbol,
      name: c.name || symbol,
...
function jitter(quotes: Quote[]): Quote[] {
  return quotes.map((q) => {
    ...
    const changePct = q.changePct + shock * 100 * 0.25;
    return { ...q, price, changePct, live: false };
  });
}
